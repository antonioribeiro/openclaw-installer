...
pre_install_checks() {
    ...
    if [ "$EUID" -eq 0 ]; then
        ...
        if [ -z "$CURRENT_SCRIPT" ]; then
            # Script is piped from stdin - minimal bootstrap mode
            echo ""
            echo -e "${YELLOW}Bootstrapping installation from stdin...${NC}"
            echo ""

            local REPO_DIR="/home/$OPENCLAW_USER/installer"
            local REPO_URL="https://github.com/antonioribeiro/openclaw-installer.git"

            # Check if repo already exists
            if [ -d "$REPO_DIR/.git" ]; then
                echo "Updating existing repository..."
                (cd "$REPO_DIR" && git fetch --depth 1 && git checkout -f main && git pull --ff-only >>"$LOG_FILE" 2>&1)
            ...
            # Run the actual install script from the cloned repository
            echo "Running installation from cloned repository..."
            echo ""
            exec su - "$OPENCLAW_USER" -c "cd \"$REPO_DIR\" && ./install.sh"
            exit $?
        fi
